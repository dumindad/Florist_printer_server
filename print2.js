...
async function printReceipt(data, ws) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const printer = new escpos.Printer(device);
      const { invoiceNo, createdAt, items, paymentMethod } = data;
      ...
      //       // Now continue after image is printed
      //       printer
      //         .style("b")
      //         .size(1, 1)
      //         .text("Marabedda Florist")
      ...
      //       if (data.discount > 0) {
      //         printer.text(
      //           `Discount: ${data.discout_symbol}${data.discount} ${data.currency}-${data.discountAmount}`
      //         );
      //       }

      //       if (data.tax > 0) {
      //         printer.text(`Tax: ${data.currency} ${data.tax}`);
      //       }

      //       printer
      //         .style("b")
      //         .text(`Total: ${data.currency} ${data.total}`)
      //         .style("normal");

      //       if (data.paymentMethod === "cash") {
      //         printer
      //           .text(`Cash Tendered: ${data.currency} ${data.cashTendered}`)
      //           .text(`Change: ${data.currency} ${data.cashBalance}`);
      //       }
